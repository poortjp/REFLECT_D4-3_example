function [new_contents] = replace_REACTION_TEMPERATURE(contents, number, value)
% Finds and replaces the REACTION_TEMPERATURE of the specified input
...
% replace the temperature values ('num2str(number)')
...
pattern = ['REACTION_TEMPERATURE\s+' num2str(number) '(\s*\d+(\.)?\d*)+'];
...
new_string = "REACTION_TEMPERATURE " + blocknum + "\n" + repval_string;

% Check whether pattern exists in contents
idxs = regexp(contents, pattern);
if isempty(idxs)
    warndlg('replace_REACTION_TEMPERATURE error: pattern not found in string')
